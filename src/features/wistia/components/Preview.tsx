        ...
        if (data.ok) {
          data.json().then(data => {
            if (data.type === 'UnknownType') {
              setPreview(false);
              setErrorMessage(
                'Looks like the video file might be corrupted.\n' +
  ...
  return (
    <>
      {!preview ? (
        ...
        >
          <Box style={{ width: '75%' }}>
            <Subheading style={{ color: tokens.red600 }}> Video not found</Subheading>
            <Paragraph style={{ color: tokens.gray600 }}>{errorMessage}</Paragraph>
          </Box>
        </Flex>
      ) : (
        <Box
