...
const Field = () => {
  ...
  const setNewThumbnail = async () => {
    ...
    const embedAssetUrl = `${media.assets[0].url.split('.bin')[0]}.jpg?video_still_time=${
      video?.time() || 0
    }`;

    //TODO: DRY - optimize Thumbnail extraction
    const newThumbnail = await fetch(
      `https://upload.wistia.com?project_id=${sdk.parameters.installation.projectId}`,
      {
        method: 'POST',
        cache: 'no-cache',
        headers,
    ...
    if (newThumbnail.hashed_id !== undefined) {
      await fetch(
        `https://api.wistia.com/v1/medias/${media.hashed_id}.json?new_still_media_id=${newThumbnail.hashed_id}`,
        {
          method: 'PUT',
          headers,
        }
  ...
  return (
    <>
      ...
      {media !== undefined && (
        <>
          <Preview media={media} />
          <Stack marginTop="spacingS" spacing="spacingM" justifyContent="space-evenly">
